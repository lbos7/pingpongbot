...
int main() {
    ...
    i2cWrite(file, motorTypeAddr, &motorType, 1);

    // Set motor encoder polarity
    uint8_t motorPolarity = 1;
    i2cWrite(file, motorEncoderPolarityAddr, &motorPolarity, 1);

    // Set motor speed (forward)
    int8_t speed[4] = {0, 0, 0, 0};  // All motors at 50 speed
    i2cWrite(file, motorFixedSpeedAddr, (uint8_t*)speed, 4);

    sleep(2);

    // Stop motors
    int8_t stop[4] = {0, 0, 0, 0};
    i2cWrite(file, motorFixedSpeedAddr, (uint8_t*)stop, 4);

    // Read encoder values
    uint8_t encoderData[16] = {0};
    i2cRead(file, motorEncoderTotalAddr, encoderData, 16);
